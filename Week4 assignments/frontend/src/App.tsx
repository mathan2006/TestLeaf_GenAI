...
import { GenerateRequest, GenerateResponse, TestCase, JiraStoryResponse } from './types'
...
function App() {
  const [formData, setFormData] = useState<GenerateRequest>({
   ...
   // Add new function to handle fetch button click
   const handleFetch = async () => {
    if (!formData.jiraStoryLink) {
      setError('Please enter a Jira story link')
      return
    }
    ...
    try {
     const response = await fetch('http://localhost:8080/api/jira/fetch-story', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
      body: JSON.stringify({ jiraStoryLink: formData.jiraStoryLink }),
    });

    if (!response.ok) {
      throw new Error('Failed to fetch Jira story');
    }

    const data: JiraStoryResponse = await response.json();
    
    // Update form data with Jira story details
    setFormData(prev => ({
      ...prev,
      ...
      <div className="container">
        ...
        <form onSubmit={handleSubmit} className="form-container">
          <div className="form-group" style={{ display: 'flex', gap: '10px' }}>
            <div style={{ flex: 1 }}>
              <label htmlFor="jiraStoryLink" className="form-label">
                Jira Story Link
              </label>
              <input
                type="text"
