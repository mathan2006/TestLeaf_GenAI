...
export async function evalWithFields(params: {
  ...
  // Output is required for most metrics, but not for conversation_completeness
  const metric = params.metric || "faithfulness";
  if (metric !== "conversation_completeness" && !params.output) {
  ...
  // Support both context and retrieval_context (retrieval_context is alias for backward compatibility)
  if (params.context) payload.context = params.context;
  if (params.retrieval_context) payload.retrieval_context = params.retrieval_context;
  
  if (params.expected_output) payload.expected_output = params.expected_output;
  if (params.messages) payload.messages = params.messages;
